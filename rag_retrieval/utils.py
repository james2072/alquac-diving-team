...
def extract_json_from_text(text: str) -> dict[str, Any] | None:
    """
    Locate and parse a valid JSON object from LLM output text.
    
    Supports nested objects/arrays and handles markdown code blocks seamlessly.
    ...
    # Step 2: Try extracting from the first '{' to the last '}'
    start_idx = text.find("{")
    end_idx = text.rfind("}")
    ...
    # Step 3: Scan balanced brace pairs from outer to inner
    pos = 0
    while True:
        start = text.find("{", pos)
        if start == -1:
